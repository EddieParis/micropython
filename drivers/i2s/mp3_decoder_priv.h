#ifndef MICROPY_INCLUDED_DRIVERS_I2S_MP3_DECODER_PRIV_H
#define MICROPY_INCLUDED_DRIVERS_I2S_MP3_DECODER_PRIV_H
...
typedef struct HuffTabLookup {
    int linBits;
    HuffTabType_t  tabType; /*HuffTabType*/
} HuffTabLookup_t;
...
inline int MY_CLZ(int x){int numZeros; if (!x) return(sizeof(int) * 8);  numZeros = 0; while (!(x & 0x80000000)){numZeros++;  x <<= 1;} return numZeros;}
...
#endif // MICROPY_INCLUDED_DRIVERS_I2S_MP3_DECODER_PRIV_H
